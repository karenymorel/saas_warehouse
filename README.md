...
4. **Analytics Marts (Views):** Business logic layer created to answer specific questions:
   - _Churn rate by geographical state._
   - _Impact of customer service calls on churn probability._
...
By default, `dev` points to a **local** DuckDB file (`dev.duckdb`) — safe to run freely without touching production data.
...
To build against the production warehouse in MotherDuck, target `prod` explicitly:
